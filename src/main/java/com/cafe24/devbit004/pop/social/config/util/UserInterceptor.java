...
//        return super.preHandle(request, response, handler);
        ...
        if (currUrl.contains("/admin/cafe24")){
            logger.info("preHandle in " + CONTEXT_PATH + "/admin/cafe24");
            return true;

        }
        if (currUrl.contains("/admin/complete")){
            logger.info("preHandle in " + CONTEXT_PATH + "/admin/complete");
            return true;

        }

        if (currUrl.contains("/admin/login")){
            logger.info("preHandle in " + CONTEXT_PATH + "/admin/login");
            return true;

        }

        if (currUrl.contains("/admin/result")) {
            logger.info("preHandle in " + CONTEXT_PATH + "/admin/result");
            return true;
        }
